<?php
// Delete News Article Confirmation Page
...
<div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
    <div>
        <h1 class="h3 fw-bold text-danger mb-0">
            <i class="bi bi-trash me-2"></i>Delete Article #<?= (int)$news['id'] ?>
        </h1>
...
<div class="alert alert-danger d-flex align-items-start gap-3 mb-4" role="alert">
    ...
    <div>
        <h6 class="fw-bold mb-1">Warning! You are about to permanently delete this news article.</h6>
        <p class="mb-0">
            All associated <strong><?= $commentCount ?> comment<?= $commentCount === 1 ? '' : 's' ?></strong> will also be deleted from the database. Data recovery is not possible.
        </p>
    </div>
</div>
    ...
    <div class="col-lg-8">
        <div class="card border-danger border-2 shadow-sm mb-4">
            <div class="card-header bg-danger bg-opacity-10 text-danger fw-semibold py-3">
                <i class="bi bi-newspaper me-2"></i>Article Summary
            </div>
            <div class="card-body p-4">
                <table class="table table-borderless mb-0">
                    <tbody>
                        ...
                        <tr>
                            <td class="text-muted fw-semibold">Headline</td>
                        ...
                        <tr>
                            <td class="text-muted fw-semibold">Comments</td>
                            <td>
                                <span class="badge <?= $commentCount > 0 ? 'bg-danger' : 'bg-secondary' ?> rounded-pill">
                                    <?= $commentCount ?> comment<?= $commentCount === 1 ? '' : 's' ?>
                                </span>
                                <?php if ($commentCount > 0): ?>
                                    <small class="text-danger ms-2">— will be permanently removed</small>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <tr>
                            <td class="text-muted fw-semibold align-top">Text Preview</td>
        ...
        <form action="index.php?action=newsDelete&id=<?= (int)$news['id'] ?>" method="POST">
            ...
            <div class="d-flex gap-3">
                ...
                <button type="submit" class="btn btn-danger rounded-pill px-5 fw-semibold shadow-sm">
                    <i class="bi bi-trash me-1"></i> Yes, Delete Article
                </button>
            </div>
        </form>
    </div>
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white fw-semibold py-3">
                <i class="bi bi-image me-2 text-muted"></i>Article Image
            </div>
            <div class="card-body p-3 text-center">
                <?php if (!empty($currentImg)): ?>
                    ...
                    <p class="text-muted small mt-2 mb-0">
                        <i class="bi bi-info-circle me-1"></i>Image will be permanently deleted
                    </p>
                <?php else: ?>
