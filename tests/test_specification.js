...
async function runTests() {
    ...
    try {
        ...
        assert(html.includes('NewsPortal') && (html.includes('Home') || html.includes('Главная')), 'TC-01.2: Brand header and navigation present');
        assert(html.includes('Latest News') || html.includes('TOP 3 NEWS') || html.includes('Последние публикации') || html.includes('news-card'), 'TC-01.3: Top news articles rendered on home page');
    } catch (err) {
    ...
    try {
        ...
        assert(resAll.status === 200 && (htmlAll.includes('All News') || htmlAll.includes('Все новости') || htmlAll.includes('Kõik uudised')), 'TC-02.1: All News catalog loads properly');

        const resCat = await fetch(`${BASE_URL}/index.php?action=category&id=1`);
        const htmlCat = await resCat.text();
        assert(resCat.status === 200 && (htmlCat.includes('Category') || htmlCat.includes('Категория')), 'TC-02.2: Category filter renders news by category');
    } catch (err) {
    ...
    try {
        ...
        assert(resRead.status === 200 && (htmlRead.includes('Comments') || htmlRead.includes('Комментарии')), 'TC-03.1: Article reading page with comments section');
    ...
    try {
        ...
        assert(htmlMismatch.includes('do not match') || htmlMismatch.includes('не совпадают') || htmlMismatch.includes('Failed') || htmlMismatch.includes('Ошибка'), 'TC-04.1: Registration detects password mismatch');
        ...
        assert(htmlDup.includes('already registered') || htmlDup.includes('уже зарегистрирован') || htmlDup.includes('Failed') || htmlDup.includes('Ошибка'), 'TC-04.2: Registration detects already registered email');
    } catch (err) {
    ...
    try {
        ...
        assert(htmlAdminUnauth.includes('Admin Sign In') || htmlAdminUnauth.includes('Вход в Админ-панель') || htmlAdminUnauth.includes('emailInput'), 'TC-06.1: Unauthenticated request to admin panel prompts login form');
